...
function ResourcePage({ slug, mode }) {
  ...
  const isPaginated = slug === 'attendance' || slug === 'employees';
  const refresh = () => {
    const load = isPaginated
  ...
  const effectivePageSize = isPaginated ? pageSize : Math.max(1, totalRecords);
